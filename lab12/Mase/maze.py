    ...
    def find_path(self):
        ...
        while not stack.is_empty():
            ...
            if self._exit_found(current_cell.row, current_cell.col):
                return True
            next_cells = [_CellPosition(current_cell.row, current_cell.col-1),
            ...
            for next_cell in next_cells:
                if self._valid_move(next_cell.row, next_cell.col):
                    ...
                    has_valid = True
                    print("has valid", current_cell.row, current_cell.col)
                else:
                    print("hasnt valid", current_cell.row, current_cell.col)
                ...
                if not has_valid:
                    ...
                    for prev_cell in next_cells:
                        if self._get_cell_value(prev_cell) == self.PATH_TOKEN:
                            print("same value", prev_cell.row, prev_cell.col)
                            if self._valid_move(prev_cell.row, prev_cell.col):
                                print("hea, here it is", prev_cell.row, prev_cell.col)
                                stack.push(prev_cell)
                                self._mark_path(current_cell.row, current_cell.col)
        return False

    def _get_cell_value(self, cell):
        """Gets value if is appropriate."""
        if cell.row >= 0 and cell.row < self.num_rows() \
               and cell.col >= 0 and cell.col < self.num_cols():
